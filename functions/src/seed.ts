...
async function main() {
  ...
  for (let i = 1; i <= 10; i++) {
    const proveedor = await prisma.proveedores.upsert({
      ...
      create: {
        ...
        descripcion: `Somos una empresa familiar dedicada a ofrecer productos de calidad. Tenemos más de ${
          5 + i
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const campaign = await prisma.compras_colectivas.create({
      data: {
        ...
        descripcion: `¡Únete y ahorra hasta ${Math.floor(descuento * 100)}% en ${
  ...
  for (const campaign of campaigns) {
    ...
    const monto_recaudado = Math.min(
      participants.reduce((sum, p) => sum + Number(p.monto_aportado), 0),
      99999999.99, // Max value for Decimal(12,2) is 10^10 - 1
    );
    const porcentaje_completado = Math.min(
      (cantidad_actual / campaign.cantidad_objetivo) * 100,
      100,
    );
    ...
    try {
      await prisma.lista_deseos.create({
        data: {
          ...
          precio_maximo: Math.floor(
            producto.precio_unitario.toNumber() * (0.8 + Math.random() * 0.4),
          ),
        },
      });
    } catch (error) {
