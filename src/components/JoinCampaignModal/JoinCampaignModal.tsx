...
export const JoinCampaignModal: React.FC<JoinCampaignModalProps> = ({
  ...
  const [cantidad, setCantidad] = useState<number>(campaign.minimum_purchase || 1);
  ...
  const handleClose = () => {
    setCantidad(campaign.minimum_purchase || 1);
    setError('');
    onClose();
  };
  ...
  const handleSubmit = () => {
    ...
    if (cantidad < (campaign.minimum_purchase || 1)) {
      setError(`La cantidad mínima requerida es ${campaign.minimum_purchase} ${campaign.producto?.unit_type === 'kg' ? 'kg' : 'unidades'}`);
      return;
    }
    ...
    if (cantidad > remaining) {
      setError(`La cantidad máxima disponible es ${remaining} ${campaign.producto?.unit_type === 'kg' ? 'kg' : 'unidades'}`);
      return;
    }
  ...
  const progressPercentage = currentProgress 
    ...
    : 0;
  const remaining = campaign.cantidad_objetivo - (currentProgress?.cantidad_actual || 0);
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, mt: 1 }}>
          ...
          <Box sx={{ p: 2, border: 1, borderColor: 'divider', borderRadius: 2 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
              <Typography variant="body2" color="text.secondary">
                {currentProgress?.cantidad_actual || 0} de {campaign.cantidad_objetivo} {campaign.producto?.unit_type === 'kg' ? 'kg' : 'unidades'}
              </Typography>
          ...
          <TextField
            fullWidth
            type="number"
            label={`Cantidad a Comprar (${campaign.producto?.unit_type === 'kg' ? 'Kilogramos' : 'Unidades'})`}
            value={cantidad}
            onChange={(e) => setCantidad(Number(e.target.value))}
            error={!!error}
            helperText={
              error || 
              `Mínimo: ${campaign.minimum_purchase || 1} ${campaign.producto?.unit_type === 'kg' ? 'kg' : 'unidades'} • Disponible: ${remaining} ${campaign.producto?.unit_type === 'kg' ? 'kg' : 'unidades'}`
            }
            required
            InputProps={{
              inputProps: { 
                min: campaign.minimum_purchase || 1,
                max: remaining,
              }
            }}
          />
          ...
          <Box sx={{ p: 2, bgcolor: 'primary.50', borderRadius: 2, border: 1, borderColor: 'primary.200' }}>
            ...
            <Typography variant="body2" color="text.secondary">
              {cantidad} {campaign.producto?.unit_type === 'kg' ? 'kilogramos' : 'unidades'} × {formatCLP(campaign.precio_objetivo)}
            </Typography>
          </Box>
          ...
          {joinCampaignMutation.isError && error && (
