...
import { isSigningOutState } from '../store/auth';
import {
  authCustomerState,
...
export const useAuth = () => {
  ...
  const clearAuthState = () => {
    console.log('🧹 Clearing auth state and navigating to home');
    setUser(null);
    setCustomer(null);
    setSupplier(null);
    setAuth({
    ...
    const {
      ...
      if (event === 'INITIAL_SESSION' || event === 'SIGNED_IN') {
        if (session?.user) {
          ...
          console.log('🚫 No session/user found, setting as initialized');
          setIsInitialized(true);
          setIsLoading(false);
        }
  ...
  //     setIsSigningOut(false); // Clear signing out state on error
  ...
  const signOut = async () => {
    ...
    try {
      ...
      console.log(
        '✅ Sign out successful - auth state listener will handle cleanup and navigation',
      );
    } catch (error) {
