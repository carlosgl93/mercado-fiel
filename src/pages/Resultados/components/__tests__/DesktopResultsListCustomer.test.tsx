...
import { DesktopResultsListCustomer } from '../DesktopResultsListCustomer';
...
describe('DesktopResultsListCustomer', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('should call navigation function when customer is clicked', () => {
    renderWithProviders([mockCustomer]);
    
    const customerItem = screen.getByText('Juan Pérez').closest('li');
    expect(customerItem).toBeInTheDocument();
    
    fireEvent.click(customerItem!);
    
    expect(mockNavigate).toHaveBeenCalledWith('/perfil-cliente/1', {
      state: {
        prestador: mockCustomer,
      },
    });
  });
  ...
  it('should display customer name with proper styling', () => {
    renderWithProviders([mockCustomer]);
