...
import { DesktopResultsListSupplier } from '../DesktopResultsListSupplier';
...
describe('DesktopResultsListSupplier', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('should call navigation function when supplier is clicked', () => {
    renderWithProviders([mockSupplier]);
    
    const supplierItem = screen.getByText('Negocio Test').closest('li');
    expect(supplierItem).toBeInTheDocument();
    
    fireEvent.click(supplierItem!);
    
    expect(mockNavigate).toHaveBeenCalledWith('/perfil-proveedor/1', {
      state: {
        proveedor: mockSupplier,
      },
    });
  });
  ...
  it('should display supplier name with proper styling', () => {
    renderWithProviders([mockSupplier]);
    
    const nameElement = screen.getByText('Negocio Test');
  ...
  it('should render button with correct text', () => {
    ...
    const button = screen.getByRole('button', { name: 'Ver perfil' });
